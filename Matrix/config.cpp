...
// Compatibility: some older SDKs don't define ICC_LINK_CLASS
...
// Get a version string from the module
TCHAR* GetVersionString(const TCHAR* szFileName, const TCHAR* szValue, TCHAR* szBuffer, ULONG nLength)
{
    ...
    if (len == 0 || (ver = malloc(len)) == 0) return NULL;

    if (GetFileVersionInfo(szFileName, 0, len, ver))
    {
        if (VerQueryValue(ver, TEXT("\\VarFileInfo\\Translation"), (LPVOID*)&codepage, &len))
        {
            wsprintf(fmt, TEXT("\\StringFileInfo\\%04x%04x\\%s"), (*codepage) & 0xFFFF, (*codepage) >> 16, szValue);
...
INT_PTR CALLBACK configdlgproc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        // Add saved messages to the combo box
...
int Configure(HWND hwndParent)
{
    ...
    // Ensure SysLink class is available
    icc.dwICC = ICC_UPDOWN_CLASS | ICC_BAR_CLASSES | ICC_LINK_CLASS;
